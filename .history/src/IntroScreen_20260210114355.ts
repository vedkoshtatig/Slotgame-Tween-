...
export class IntroScreen extends PIXI.Container{
    ...
    build(){
        const gameLogo  = new PIXI.Sprite(Assets.get());
        gameLogo.scale.set(0.119)
        gameLogo.anchor.set(0.5,0)
        gameLogo.position.set(this.app.screen.width/2,-3)
        
        const dispScreen = new PIXI.Sprite(this.textures.IntroScreen1);
        ...
        const playBtn =  new PIXI.Sprite(this.textures.playBtnNormal)
        ...
        lBtn.on("pointerdown" , () =>{
           if(this.screenState===0){
             dispScreen.texture = this.textures.IntroScreen2
             leftInd.texture=this.textures.idleIndicator
             rightInd.texture=this.textures.activeIndicator
             this.screenState=1;
           }
           else if(this.screenState===1){
             dispScreen.texture = this.textures.IntroScreen1
             leftInd.texture=this.textures.activeIndicator
             rightInd.texture=this.textures.idleIndicator
             this.screenState=0
           }
        });
