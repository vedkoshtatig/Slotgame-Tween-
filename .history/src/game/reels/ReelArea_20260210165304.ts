...
public startSpin() {
  ...
  for (let i = 0; i < this.reels.length; i++) {
    ...
    const originalY = r.position;
    const spinDistance = 600; // how far it moves
    const downTarget = originalY + spinDistance;
...
private update() {
  ...
  for (let i = 0; i < this.reels.length; i++) {
    ...
    r.container.y = r.position;

    r.blur.blurY = (r.position - r.previousPosition) * 0.3;
