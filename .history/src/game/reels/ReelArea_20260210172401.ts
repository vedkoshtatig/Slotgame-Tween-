...
export class ReelArea extends PIXI.Container {
  ...
  // Move symbol down based on reel speed
  const speed = (r.position - r.previousPosition) * 10;
  s.y += speed;

  const totalHeight =
    this.totalRows * (this.symbolSize + this.symbolGapY);

  // If symbol goes below bottom → move to top
  if (s.y >= totalHeight) {
    s.y -= totalHeight;

    // When symbol respawns at top, apply result if needed
    this.applyResult(i);
  }
}
