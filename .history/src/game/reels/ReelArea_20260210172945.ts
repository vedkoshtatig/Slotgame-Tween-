...
export class ReelArea extends PIXI.Container {
  ...
  private symbolHeight: numberz
  private virtualIndex: number
  private reelCount = 5;
  ...
  for (let row = 0; row < this.visibleRows; row++) {
    ...
    // +1 because first buffer row is hidden
  ...
  private update() {
    ...
    for (let i = 0; i < this.reels.length; i++) {
      ...
      for (let j = 0; j < r.symbols.length; j++) {
        ...
        const prevY = s.y;
//Respawn at top
this.symbolHeight = this.symbolSize + this.symbolGapY;

// 1. Calculate virtual row index
this.virtualIndex = (r.position + j) % this.totalRows;

// 2. Convert row index to pixel position
this.yPosition = virtualIndex * symbolHeight;

// 3. Shift up by one row (because of buffer)
s.y = this.yPosition - symbolHeight;
