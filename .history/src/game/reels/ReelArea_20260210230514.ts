...
private isTurbo = false;
  ...
  private checkWins() {
  const wins: { row: number; symbol: number }[] = [];

  for (let row = 0; row < this.visibleRows; row++) {
    const firstSymbol = this.resultMatrix[0][row];
    let isWinning = true;

    for (let reel = 1; reel < this.reelCount; reel++) {
      if (this.resultMatrix[reel][row] !== firstSymbol) {
        isWinning = false;
        break;
      }
    }
    ...
    if (isWinning) {
      wins.push({ row, symbol: firstSymbol });
    }
  }

  return wins;
}
private animateWins(wins: { row: number; symbol: number }[]) {
  for (const win of wins) {
    for (let reelIndex = 0; reelIndex < this.reelCount; reelIndex++) {
      ...
      // Pulse animation
      this.tweenTo(symbol.scale, "x", 1.2, 400, this.backout(0.5));
      this.tweenTo(symbol.scale, "y", 1.2, 400, this.backout(0.5), () => {
        this.tweenTo(symbol.scale, "x", 1, 400, this.backout(0.5));
        this.tweenTo(symbol.scale, "y", 1, 400, this.backout(0.5));
      });
    }
  }
}
