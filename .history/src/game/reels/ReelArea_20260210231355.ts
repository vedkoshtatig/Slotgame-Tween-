...
private isTurbo = false;
  ...
  private spawnRandomReels() {
    ...
    const centerY =
      ...
      (this.visibleRows * (this.symbolSize + this.symbolGapY)) / 2+10;
  ...
  private checkWins() {
  const wins: { reel: number; row: number }[][] = [];

  for (const line of this.paylines) {
    let count = 1;
    let tempWin: { reel: number; row: number }[] = [];

    let prevSymbol = this.resultMatrix[0][line[0]];
    tempWin.push({ reel: 0, row: line[0] });
    ...
    for (let reel = 1; reel < this.reelCount; reel++) {
      ...
      if (currentSymbol === prevSymbol) {
        count++;
        tempWin.push({ reel, row: line[reel] });
...
private animateWins(wins: { reel: number; row: number }[][]) {

  // 1️⃣ Dim everything first
  this.dimAllSymbols();

  for (const winGroup of wins) {
    for (const win of winGroup) {
      const reel = this.reels[win.reel];
      const symbol = reel.symbols[win.row + 1];

      // Highlight winner
      symbol.alpha = 1;

      // Optional glow
      const glow = new PIXI.BlurFilter();
      glow.blurY = 4;
      symbol.filters = [glow];

      // Pulse animation
      this.tweenTo(symbol.scale, "x", 1.2, 300, this.backout(0.5));
      this.tweenTo(symbol.scale, "y", 1.2, 300, this.backout(0.5), () => {
        this.tweenTo(symbol.scale, "x", 1, 300, this.backout(0.5));
        this.tweenTo(symbol.scale, "y", 1, 300, this.backout(0.5));
      });
    }
  }

  // 2️⃣ Restore after delay
  setTimeout(() => {
    this.restoreAllSymbols();
  }, 1500);
}
