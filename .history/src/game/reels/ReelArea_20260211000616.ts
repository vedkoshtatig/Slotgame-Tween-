...
private highlightTimeout?: number;
  ...
  const reel = this.reels[reelIndex];
  const symbol = reel.symbols[row + 1]; // +1 because of buffer
  return this.normalTextures.indexOf(symbol.texture);
}

private checkWins() {
  const winningPositions: { reel: number; row: number }[] = [];

  for (let row = 0; row < this.visibleRows; row++) {
    let matchCount = 1;
    let firstSymbolIndex = this.getSymbolIndex(0, row);

    for (let reel = 1; reel < this.reelCount; reel++) {
      const currentIndex = this.getSymbolIndex(reel, row);

      if (currentIndex === firstSymbolIndex) {
        matchCount++;
      } else {
        break; // stop when chain breaks
      }
    }

    if (matchCount >= 3) {
      for (let r = 0; r < matchCount; r++) {
        winningPositions.push({ reel: r, row });
      }
    }
  }

  if (winningPositions.length > 0) {
    this.highlightWins(winningPositions);
  }
}
private highlightWins(
  ...
  for (let i = 0; i < this.reels.length; i++) {
    ...
    for (let j = 0; j < r.symbols.length; j++) {
      r.symbols[j].alpha = 0.3;
      r.symbols[j].scale.set(1);
    }
  }

  // Highlight winners
  winners.forEach(pos => {
    const symbol = this.reels[pos.reel].symbols[pos.row + 1];
