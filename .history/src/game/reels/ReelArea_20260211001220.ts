...
private checkWins() {
  for (const line of this.paylines) {
    ...
    if (matchCount >= 3) {
      this.animateProgressiveWin(line, matchCount);
      return; // stop after first win (optional)
    }
  }
}
private animateProgressiveWin(line: number[], totalMatches: number) {
  let currentCount = 3;
  ...
  const step = () => {
    ...
    if (currentCount < totalMatches) {
      ...
      setTimeout(step, 1000); // delay between 3 → 4 → 5
    } else {
      setTimeout(() => {
